...
namespace mdb {
const adt::vector<MovieEntry> &DataReader::get_entries() const {
  return entries;
}
void DataReader::open_file(const std::string &filename) {
...
void DataReader::read_data(const std::string &filename) {
  open_file(filename);
...
u_int DataReader::count_lines_in_file() {
...
void DataReader::read_entry(const std::string &line) {
  MovieEntry entry{};
  try {
...
MovieEntry DataReader::parse_entry(const std::string &line)  {
